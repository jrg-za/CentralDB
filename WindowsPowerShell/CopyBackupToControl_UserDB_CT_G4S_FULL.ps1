...
# Get backups less than 12 hours old from CashTrace_G4S FULL 
Get-ChildItem I:\Backups\ASPRODDB-BUR01\CashTrace_G4S\FULL |
  Where-Object { $_.PSIsContainer -eq $false -and $_.LastWriteTime -gt ($d.AddHours(-12))} |
    ForEach-Object { Copy-ItemUNC -SourcePath $_.DirectoryName -TargetPath \\197.96.167.80\ASPRODDB-BUR01$\CashTrace_G4S\FULL -FileName $_.Name}
# Remove backups more than 8 days old from CashTrace_G4S FULL  
Get-ChildItem '\\197.96.167.80\ASPRODDB-BUR01$\CashTrace_G4S\FULL' -ErrorAction SilentlyContinue | 

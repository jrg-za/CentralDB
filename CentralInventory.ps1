...
###################################################################################################################################
param(
	[string]$SQLInst="LOCALHOST\SQLEXPRESS",
...
elseif($a.Caption  -Like '*2008*'){$OSName = 'Windows Server 2008'} elseif($a.Caption -Like '*2012 R2*'){$OSName = 'Windows Server 2012 R2'} 
elseif($a.Caption -Like '*2012*'){$OSName = 'Windows Server 2012'} else{$OSName = 'Unknown'}
...
if(($s.Version -Like '8.*') -and ($s.ProductLevel -eq 'SP4')){ $IsSPUpToDate = 'True'} elseif(($s.Version -Like '9.*') -and ($s.ProductLevel -eq 'SP4')){ $IsSPUpToDate = 'True'} elseif(($s.Version -Like '10.5*') -and ($s.ProductLevel -eq 'SP2')){ $IsSPUpToDate = 'True' } 
elseif(($s.Version -Like '10.*') -and ($s.ProductLevel -eq 'SP3')){  $IsSPUpToDate = 'True' } elseif(($s.Version -Like '11.0*') -and ($s.ProductLevel -eq 'SP1')){ $IsSPUpToDate = 'True'} 
...
$dt= $s | Select @{n="ServerName";e={$svr}}, @{n="InstanceName";e={$inst}},  @{n="IPAddress";e={$ip}}, @{n="Port";e={$port}}, @{n="SQLVersion";e={$SQLVersion}},
	ProductLevel,@{n="IsSPUpToDate";e={$IsSPUpToDate}}, @{n="SQLEdition";e={$SQLEdition}}, Version, Collation, RootDirectory, DefaultFile, DefaultLog, 
	...
	ClusterQuorumType, HadrManagerStatus,@{n="MaxMemory";e={$_.Configuration.MaxServerMemory.RunValue}}, @{n="MinMemory";e={$_.Configuration.MinServerMemory.RunValue}}, 
	@{n="MaxDOP";e={$_.Configuration.MaxDegreeOfParallelism.RunValue}}, @{n="NoOfUsrDBs";e={($_.Databases.Count)-4}}, @{n="NoOfJobs";e={$_.JobServer.Jobs.Count}}, 
	...
	@{n="NoOfAvailGroups";e={$_.AvailabilityGroups.Count}}, @{n="DateAdded";e={$RunDt}} | out-datatable
Write-DataTable -ServerInstance $SQLInst -Database $Centraldb -TableName $CITbl -Data $dt
}
catch 
{ 
...
$dt= $db | Select @{n="ServerName";e={$svr}}, @{n="InstanceName";e={$inst}}, Name, Status, Owner, CreateDate, Size, 
	...
	@{n="NoOfUsers";e={$_.Users.Count}}, @{n="NoOfDBTriggers";e={$_.Triggers.Count}}, @{n="DateAdded";e={$RunDt}}  | out-datatable	
Write-DataTable -ServerInstance $SQLInst -Database $Centraldb -TableName $CITbl -Data $dt
}}#end For Each
}
catch 
...
if(($s.Version -Like '8.*') -and ($s.ProductLevel -eq 'SP4')){ $IsSPUpToDateOnAS = 'True'} elseif(($s.Version -Like '9.*') -and ($s.ProductLevel -eq 'SP4')){ $IsSPUpToDateOnAS = 'True'} elseif(($s.Version -Like '10.5*') -and ($s.ProductLevel -eq 'SP2')){ $IsSPUpToDateOnAS = 'True' } 
elseif(($s.Version -Like '10.*') -and ($s.ProductLevel -eq 'SP3')){  $IsSPUpToDateOnAS = 'True' } elseif(($s.Version -Like '11.0*') -and ($s.ProductLevel -eq 'SP1')){ $IsSPUpToDateOnAS = 'True'} 
...
$q = "Select Distinct ServerName, InstanceName from [Svr].[ServerList] where Inventory='True';"

...
$Proc = get-Counter "\\$svr\Processor(_total)\% Processor Time"
$PctProcTm=$proc.counterSamples[0].CookedValue

$ProcQ = get-Counter "\\$svr\System\Processor Queue Length"
$ProcQLen = $ProcQ.counterSamples[0].CookedValue

#Disk Counters

$dskRd = get-Counter "\\$svr\PhysicalDisk(_total)\Avg. Disk sec/Read"
$AvDskRd = $dskRd.counterSamples[0].CookedValue

$dskWt = get-Counter "\\$svr\PhysicalDisk(_total)\Avg. Disk sec/Write"
$AvDskWt = $dskWt.counterSamples[0].CookedValue

$dskQ = get-Counter "\\$svr\PhysicalDisk(_total)\Avg. Disk Queue Length"
$AvDskQLen = $dskQ.counterSamples[0].CookedValue

#Memory Counters

$AvlMB = get-Counter "\\$svr\Memory\Available MBytes"
$AvailMB = $AvlMB.counterSamples[0].CookedValue

$PgFl = get-Counter "\\$svr\Paging File(_total)\% Usage"
$PgFlUsg = $PgFl.counterSamples[0].CookedValue

$dt = get-Counter | select @{n="ServerName";e={$svr}}, @{n="InstanceName";e={$inst}}, @{Name="RunDate"; Expression = {$Date}}, 
...
$q = " Select Distinct ServerName, InstanceName from [Svr].[ServerList] where Baseline='True';"
